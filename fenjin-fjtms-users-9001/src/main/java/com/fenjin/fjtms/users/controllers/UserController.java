...
public class UserController extends BaseController {
    ...
    public CommonResult create(@Valid @RequestBody User user, BindingResult bindingResult) {

        // 后续问题：前台除了提供用户基本信息，也需要提供该用户的用户角色，用户所属部门等信息。

        if (bindingResult.hasErrors()) {
            FieldError error = (FieldError) bindingResult.getAllErrors().get(0);
            return new CommonResult().validateFailed(error.getDefaultMessage());
        }
        try {
            ...
            if(!StringUtil.isEmpty(user.getEmail()) && userService.getUserByEmail(user.getEmail()) != null){
                return new CommonResult().validateFailed("邮箱已存在");
            }
    ...
    @ApiImplicitParam(paramType="body", name = "user", value = "有效的用户实例", required = true, dataType = "User")
    public CommonResult edit(@Valid @RequestBody User user, BindingResult bindingResult) {
    ...
    public CommonResult list(@RequestBody(required = false) UserSearchModel userSearchModel, @PathVariable Integer pageIndex, @PathVariable Integer pageSize) {
